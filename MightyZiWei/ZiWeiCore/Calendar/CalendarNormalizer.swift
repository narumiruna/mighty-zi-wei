...
public enum CalendarNormalizer {
    public static func normalize(_ profile: BirthProfile) throws -> NormalizedBirth {
        ...
        guard (0...23).contains(profile.localTime.hour),
              (0...59).contains(profile.localTime.minute) else {
        ...
        guard let noon = calendar.date(from: noonComponents),
              sameDate(calendar.dateComponents([.year, .month, .day], from: noon), date) else {
        ...
        guard let first = calendar.nextDate(
            ...
            direction: .forward
        ), matches(calendar: calendar, instant: first, profile: profile) else {
        ...
        let repeated = last.map { $0 != first && matches(calendar: calendar, instant: $0, profile: profile) } ?? false
    ...
    public static func lunarDate(from normalizedBirth: NormalizedBirth) throws -> LunarDate {
        ...
        calendar.timeZone = timeZone
        let values = calendar.dateComponents([.year, .month, .day, .isLeapMonth], from: normalizedBirth.instant)
