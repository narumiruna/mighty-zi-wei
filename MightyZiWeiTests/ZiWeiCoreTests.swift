...
final class ZiWeiCoreTests: XCTestCase {
    ...
    func testGoldenFixtures() throws {
        for name in fixtureNames {
            ...
            XCTAssertEqual(chart.lifePalace.stemBranch.branch.rawValueName, fixture.expected.lifeBranch, name)
            XCTAssertEqual(chart.bodyPalace.stemBranch.branch.rawValueName, fixture.expected.bodyBranch, name)
            ...
            for (starName, branchName) in fixture.expected.stars {
                ...
                XCTAssertEqual(chart.placement(of: star).branch.rawValueName, branchName, "\(name): \(starName)")
            }
    ...
    func testZiHourUsesCivilDateWithoutEarlyRollover() throws {
        ...
        XCTAssertEqual(chart.lunarDate, LunarDate(cyclicalYear: 16, month: 11, day: 25, isLeapMonth: false))
    }
    ...
    func testRejectsInvalidAndNonexistentLocalTimes() {
        XCTAssertThrowsError(try CalendarNormalizer.normalize(BirthProfile(
    ...
    func testRepeatedLocalTimeUsesFirstOccurrence() throws {
        let normalized = try CalendarNormalizer.normalize(BirthProfile(
    ...
    private func assertRelations(_ chart: ZiWeiChart, fixtureName: String) {
        let palaceByBranch = Dictionary(uniqueKeysWithValues: chart.palaces.map { ($0.stemBranch.branch.rawValue, $0.kind) })
        for palace in chart.palaces {
            let relation = chart.relation(of: palace.kind)
            let branch = palace.stemBranch.branch.rawValue
            XCTAssertEqual(relation.trines, [palaceByBranch[(branch + 4) % 12]!, palaceByBranch[(branch + 8) % 12]!], fixtureName)
    ...
    private func loadFixture(named name: String) throws -> GoldenFixture {
        ...
        return try JSONDecoder().decode(GoldenFixture.self, from: Data(contentsOf: try XCTUnwrap(url, name)))
    }
}
