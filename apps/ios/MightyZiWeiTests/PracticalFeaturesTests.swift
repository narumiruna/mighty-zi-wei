...
final class PracticalFeaturesTests: XCTestCase {
  ...
  func test命盤助理輸入狀態會說明所有停用原因與字數邊界() {
    ...
    XCTAssertEqual(
      policy.blockReason(
        draft: "   ", isRequesting: false, hasReachedRoundLimit: false, isVoiceActive: false),
      .empty
    )
    XCTAssertNil(
      policy.blockReason(
    ...
    XCTAssertEqual(
      policy.blockReason(
        draft: "問題", isRequesting: true, hasReachedRoundLimit: false, isVoiceActive: false),
      .requesting
    )
    XCTAssertEqual(
      policy.blockReason(
        draft: "問題", isRequesting: false, hasReachedRoundLimit: true, isVoiceActive: false),
      .roundLimit
    )
    XCTAssertEqual(
      policy.blockReason(
        draft: "問題", isRequesting: false, hasReachedRoundLimit: false, isVoiceActive: true),
      .voiceActive
    )
  ...
  func test回答後續建議依狀態產生且不需要模型() {
    ...
    XCTAssertTrue(
      builder.make(for: .unsupported).allSatisfy { $0.contains("個性") || $0.contains("工作") })
  }
  ...
  func testCloudKit收藏驗證Seed與Fact完整配對並保留舊資料例外() {
    ...
    XCTAssertTrue(
      CloudInsightPayload(matching).hasValidEvidence(
        seeds: [seed],
        validFactIDs: validFactIDs
      ))
    XCTAssertFalse(
      CloudInsightPayload(mismatched).hasValidEvidence(
        seeds: [seed],
        validFactIDs: validFactIDs
      ))
    XCTAssertTrue(
      CloudInsightPayload(legacy).hasValidEvidence(
        seeds: [seed],
        validFactIDs: validFactIDs
      ))
  }
