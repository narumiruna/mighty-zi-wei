...
final class AdjacentHourComparisonBuilderTests: XCTestCase {
    func test比較只包含指定的已驗證位置差異() throws {
        let comparison = try AdjacentHourComparisonBuilder().make(from: BirthProfile(
    ...
    func test子時跨日仍取得正確的前後時辰() throws {
        let lateZiHour = try AdjacentHourComparisonBuilder().make(from: BirthProfile(
        ...
        XCTAssertEqual(lateZiHour.previous.birthProfile.localDate, LocalDate(year: 2000, month: 1, day: 1))
        ...
        let earlyZiHour = try AdjacentHourComparisonBuilder().make(from: BirthProfile(
        ...
        XCTAssertEqual(earlyZiHour.previous.birthProfile.localDate, LocalDate(year: 2000, month: 1, day: 1))
    ...
    func test夏令時間跳時以相鄰民用時辰計算() throws {
        ...
        XCTAssertEqual(comparison.previous.birthProfile.localDate, LocalDate(year: 2021, month: 3, day: 13))
    ...
    func test支援日期邊界外的相鄰時辰會回傳明確錯誤() {
        XCTAssertThrowsError(try AdjacentHourComparisonBuilder().make(from: BirthProfile(
