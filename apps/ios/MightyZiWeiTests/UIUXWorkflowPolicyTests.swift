...
final class UIUXWorkflowPolicyTests: XCTestCase {
    func test已儲存命盤依搜尋與篩選條件區分結果狀態() {
        ...
        XCTAssertEqual(policy.state(hasResults: true, searchText: "不存在", hasActiveFilters: true), .results)
        XCTAssertEqual(policy.state(hasResults: false, searchText: "小明", hasActiveFilters: false), .searchEmpty)
        XCTAssertEqual(policy.state(hasResults: false, searchText: " \n ", hasActiveFilters: true), .filterEmpty)
        XCTAssertEqual(policy.state(hasResults: false, searchText: "小明", hasActiveFilters: true), .searchAndFilterEmpty)
