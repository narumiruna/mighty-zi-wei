...
final class ChartEncyclopediaCatalogTests: XCTestCase {
  ...
  func test每個項目附上對應的RULESET排盤章節() {
    for entry in ChartEncyclopediaCatalog.starEntries {
      ...
      case .star(let star) where star.category == .main:
    ...
    XCTAssertTrue(
      ChartEncyclopediaCatalog.palaceEntries.allSatisfy {
        $0.placementRuleSection == "RULESET.md 第 7 節〈命宮、身宮與十二宮〉"
      })
    XCTAssertTrue(
      ChartEncyclopediaCatalog.transformationEntries.allSatisfy {
  ...
  func test星曜與宮位說明標示為現行產品文案而非RULESET規則() {
    let editorialEntries =
      ChartEncyclopediaCatalog.starEntries
      + ChartEncyclopediaCatalog.palaceEntries
    XCTAssertTrue(
      editorialEntries.allSatisfy {
  ...
  func testCatalog只引用既有Domain項目() {
    XCTAssertEqual(
      ChartEncyclopediaCatalog.starEntries.compactMap { entry -> Star? in
        guard case .star(let star) = entry.reference else { return nil }
        return star
      },
      Star.allCases
    )
    XCTAssertEqual(
      ChartEncyclopediaCatalog.palaceEntries.compactMap { entry -> PalaceKind? in
        guard case .palace(let palace) = entry.reference else { return nil }
        return palace
      },
      PalaceKind.allCases
    )
    XCTAssertEqual(
      ChartEncyclopediaCatalog.transformationEntries.compactMap { entry -> TransformationKind? in
        guard case .transformation(let transformation) = entry.reference else { return nil }
