...
struct OpenAIResponsesInterpreter: Sendable {
  ...
  private func makeConversationPrompt(
    ...
    return """
      ...
      只有整個問題都無法根據現有資料回答，或要求健康、投資、法律建議或確定事件預測時，status 才回傳 unsupported；此時兩種 evidence ID 都必須為空陣列，answer 使用簡短拒答，App 會改用本機安全文案顯示可改問方向。
...
extension ChartConversationAnswer.Status {
  fileprivate static var allRawValues: [String] {
