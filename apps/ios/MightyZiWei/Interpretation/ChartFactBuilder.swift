...
struct ChartFactBuilder: Sendable {
  ...
  private func palaceFact(_ palace: ChartPalace) -> ChartFact {
    ChartFact(
      ...
      displayText:
        "\(palace.kind.displayName)位於\(palace.stemBranch.branch.displayName)宮，宮干支為\(palace.stemBranch.displayName)。"
    )
  }
  ...
  private func transformationFact(_ transformation: Transformation) -> ChartFact {
    ChartFact(
      ...
      displayText:
        "\(transformation.star.displayName)\(transformation.kind.displayName)，位於\(transformation.palace.displayName)。"
    )
  }
