  ...
  var body: some View {
    Form {
      ...
      Section("資料與費用") {
        ...
        Text(
          "App 不會主動把姓名、原始出生日期或出生時間加入 prompt；你在問題中輸入的內容仍會傳送。API key 只會作為授權 header 傳送到你設定的 endpoint。")
      }
    ...
    .onChange(of: commitCoordinator.credentialsTemporarilyUnavailable) {
      wasUnavailable, isUnavailable in
