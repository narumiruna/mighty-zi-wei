  ...
  var body: some View {
    List {
      ...
      Section("隱私") {
        ...
        Text("你主動使用雲端 AI 時，已驗證的命盤事實、基礎解讀、問題與本次對話會傳送到你設定的第三方 API。")
        Text("第三方服務的隱私政策、資料保存方式與費用規則由你選擇的服務決定。")
        Text("API key 儲存在這台裝置的 Keychain；未主動保存的命盤助理對話不會永久保留，已保存副本只留在本機且不自動同步。")
        Text("完整基本解讀不需要 API 或網路。")
      ...
      Section("支援") {
        Link(
          "隱私權說明",
          destination: URL(
            string: "https://github.com/narumiruna/mighty-zi-wei/blob/main/docs/PRIVACY.md")!
        )
        Link(
          "回報問題",
