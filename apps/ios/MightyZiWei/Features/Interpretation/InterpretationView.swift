...
struct InterpretationView: View {
  ...
  init(
    ...
    _displayState = State(
      initialValue: InterpretationDisplayState(
  ...
  private func sourceChoice(
    ...
    Button {
      ...
      HStack(spacing: 8) {
        Image(
          systemName: displayState.selectedSource == source ? "checkmark.circle.fill" : systemImage)
        VStack(alignment: .leading, spacing: 2) {
          Text(title)
...
extension InterpretationSection {
  fileprivate var leadingSummary: String {
    content
      .components(separatedBy: "\n\n")
      .first?
