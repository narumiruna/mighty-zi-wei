...
struct ChartAssistantView: View {
  ...
  private func loadDefaultChartIfNeeded() {
    ...
    let preferred =
      assistantStore.lastSelectedSavedChartID.flatMap { identifier in
  ...
  private func reconcileSavedCharts(
    ...
    guard previous.first(where: { $0.id == selectedID }) != currentSnapshot,
      let savedChart = savedCharts.first(where: { $0.id == selectedID })
    else {
      return
    }
...
extension ChartAssistantView {
  fileprivate enum PendingTransition {
    case clear
    case switchChart(ChartAssistantChart)
  }

  fileprivate enum TransitionError: Error {
    case saveFailed
  }
}
