...
命盤助理第一屏會持續說明送出後將傳送問題、本次對話與必要命盤依據，而且可能產生第三方費用。

使用者點選送出後，App 會直接傳送文字問題、已驗證的命盤事實、基礎解讀、本次對話與 prompt，不再顯示額外確認頁面。

AI 整理前仍會顯示傳送預覽；facts、seeds 與 model 等技術資訊由使用者主動展開。
