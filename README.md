...
- 輸入公曆或農曆出生日期、出生地當地民用時間與 IANA 時區；農曆須確認轉換後的公曆資料才會排盤。
...
- 從命盤次要入口使用命宮、主星、三方四正、解讀依據四步導覽，可跳過、續讀與重開，不增加 AI 請求。
- 由解讀、問答與收藏主動開啟本機來源卡，分開段落引用、現行產品規則、古籍定位與專家待審狀態。
...
- 另以不可覆寫的單段觀察快照保留初步想法、當時文字與依據，後續回顧獨立追加，可明確刪除而不計算準確率。
...
- 以 AES-GCM 和隨機 256-bit 復原金鑰加密匯出命盤、筆記、收藏、觀察與回顧；新備份 payload v3 保留 v1／v2 讀取支援。
- 觀察與回顧同步另行揭露並確認，使用獨立 CloudKit record types，不沿用舊同步同意。
- 不需要建立 App 帳號、開發者控制的後端或 App 訂閱；選擇性 iCloud 同步使用裝置已登入的 Apple ID。

新三合派身宮、四化與支持／牽制組合仍待來源與人工審閱，不會因增加來源卡而自動啟用。
來源與相容性界線記錄於 [`docs/interpretation/sanhe-claim-ledger.md`](docs/interpretation/sanhe-claim-ledger.md) 與 [`compatibility.md`](docs/interpretation/compatibility.md)。
Foundation 若在農曆月界回傳無效日期，農曆入口會安全拒絕，不靜默修正既有公曆排盤；已知異常與驗收限制見 [`lunar-input.md`](docs/interpretation/lunar-input.md)。
...
筆記、自我觀察標記、收藏、觀察快照、回顧與使用者主動保存的 AI 對話預設只保存在本機。

只有使用者主動開啟選擇性 iCloud 同步後，命盤、筆記與收藏才會進入 Apple 私人 CloudKit 資料庫；AI 對話、API 設定與 API key 不會同步。

使用者主動建立加密備份時，App 匯出命盤 source-of-truth、筆記、收藏、觀察快照與回顧，不匯出 API 設定、完整 AI 對話或衍生命盤快取。
觀察與回顧只有另外同意時才會同步，不進入 AI prompt、命盤分享或 Widget。
