...
輸入頁提供公曆與農曆模式，農曆年使用「該農曆新年所在的公曆年」，並明確選擇是否為閏月。
農曆輸入在本機轉成公曆，使用者確認日期、完整時間與出生地時區後才能排盤或比較相鄰時辰。
切換模式保留兩份草稿，不靜默變更日期；無效草稿及錯誤訊息保留到使用者更正。
`BirthProfile` 仍只儲存確認過的公曆資料，農曆模式與草稿不永久保存。
App 再將公曆資料轉換為排盤需要的農曆日期，沿用既有閏月與當地民用時間政策。
Foundation 月界異常造成無法核對時安全拒絕，不以輸入功能修正排盤 v1；詳見 `docs/interpretation/lunar-input.md`。
...
備份 payload schema version 目前為 3，新增觀察快照與回顧集合，並保留 v1、v2 的明確遷移路徑。
Version 2、3 的每筆收藏與筆記必須明確包含 `evidenceSeedIDs`；只有 version 1 允許缺少此欄位，未知版本一律拒絕。
`SavedObservation` 與 `SavedObservationReview` 分開保存不可覆寫的起始快照與後續回顧，不改寫原始文字、初步想法或當時依據。
快照記錄當時 facts、seed meanings、內容來源、已知內容版本與排盤版本；缺少版本的舊資料不能回填歷史認證。
回顧使用獨立 UUID，可追加更正或明確刪除，不以符合、不符合及尚無法判斷計算命理準確率。
同一 ID 的不可變內容衝突時拒絕還原或同步，不以較新時間覆寫；舊備份沒有觀察集合不表示要刪除現有觀察。
...
備份包含 `SavedChart` source-of-truth、`SavedInsight`、`SavedObservation` 與 `SavedObservationReview`，不得包含 API key、endpoint、模型名稱、完整 AI 對話或 `ZiWeiChart` cache。
...
* 新觀察與回顧同步使用獨立 record types，必須另外揭露及確認，不沿用原同步開關推定同意。
* 私人快照及回顧不得加入 AI prompt、命盤分享、Widget 或診斷紀錄。
...
「開始觀察」另外保存使用者選取的一段本機基本解讀及初步想法，後續回顧與原始快照並列，不覆寫起始內容。
通知權限拒絕或排程失敗仍應保存觀察並顯示警告；資料儲存失敗不能取消既有有效提醒。
...
選擇性 iCloud 同步預設關閉，原有同意範圍包含命盤、標籤、釘選、筆記、回顧日期與收藏。
觀察快照與後續回顧須另外同意才會同步。
...
既有命盤、筆記與收藏的 CloudKit 衝突以 `updatedAt` 較新的內容為準，刪除使用 tombstone 傳播。
觀察與回顧的不可變內容衝突必須拒絕；不同 UUID 的回顧並存，刪除使用獨立 record type 並套用父命盤 tombstone。
...
來源卡使用獨立解讀內容版本 `1`，區分產品契約、原始文本、現代轉譯與專家待審，不以段落引用宣稱 AI 逐句語意已證明。
舊收藏與保存對話沒有來源版本時只能封存閱讀，不補造歷史認證；外部連結由使用者主動開啟。
四步導覽從命盤次要入口進入，依序呈現命宮、主星、三方四正與解讀依據，不新增主分頁或 AI 請求。
導覽可跳過、續讀、往返與完成後重開；已儲存命盤的本機進度依命盤身分、內容指紋、ruleset 與導覽版本區分，刪除命盤一併清除。
未儲存命盤只保留目前畫面的進度，空宮與未核准含義維持 facts-only。
新身宮、生年四化與支持／牽制組合仍待來源及人工核准，五分類 renderer 與既有十九項 seeds 不因此擴張。
有限試點與人工 20 張命盤品質 gate 尚未完成，不以四項獨立工程功能或結構 checker 分數冒充五項功能全部驗收。
來源帳及資料相容矩陣見 `docs/interpretation/`，實際完成證據追蹤於本次計畫。
