...
命盤總覽以「閱讀命盤解讀」作為完整閱讀入口，以「問命盤助理」作為針對目前命盤提問的入口。

命盤助理必須清楚顯示目前使用的命盤，並允許使用者切換已儲存命盤。

命盤解讀第一屏必須顯示目前是完整基本解讀或已完成格式、安全與引用依據檢查的 AI 整理版。

App 必須要求模型只改寫既有解讀文字，不得改變命盤或加入 App 未提供的新命理含義，並明示目前檢查無法保證模型不改變語意。
...
已確認送出的問答在切換 App 內底部分頁後繼續執行，只有明確停止、確認清除、確認切換命盤或 App process 終止才取消。
...
App 必須指示模型只能整理、摘要或改寫這些 seeds，不得新增 seeds 未提供的命理含義。
...
命盤助理第一屏必須持續揭露點選送出後將傳送問題、本次對話與必要命盤依據，而且可能產生第三方費用。

使用者點選送出後直接建立問答請求，不再顯示額外確認頁面。

AI 整理前必須顯示傳送預覽。

AI 整理預覽第一層顯示 App 對模型只整理既有文字的要求、無法保證語意不變的限制、必要命盤依據與一次本機請求影響。

facts、seeds、model、數量與不會傳送的欄位放在 AI 整理預覽中由使用者主動展開的詳細資料。
